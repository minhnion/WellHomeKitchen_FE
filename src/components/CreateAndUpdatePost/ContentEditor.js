...
const ContentEditor = forwardRef(
  ({ content = [], onChangeContent, onOpenImageModal }, ref) => {
    ...
    const debouncedUpdateParent = useCallback(
      (structuredData) => {
        ...
        lastUpdateRef.current = setTimeout(() => {
          onChangeContent?.(structuredData);
        }, 300);
      },
      [onChangeContent]
    );
      ...
      const initialize = async () => {
        ...
        try {
          ...
          if (content && content.length > 0) {
            const initialDelta = structuredContentToDelta(content);
    ...
    useEffect(() => {
      ...
      if (!content || content.length === 0) {
        ...
        if (JSON.stringify(structuredFromEditor) !== JSON.stringify(content)) {
          const newDelta = structuredContentToDelta(content);
